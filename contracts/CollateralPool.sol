...
contract CollateralPool is AccessControlEnumerable, Pausable {
    ...
    uint256 constant public MAX_VALUE = type(uint256).max;
    ...
    struct LiquidationThresholdInfo {
        uint256 balancedLoanThreshold;
        uint256 unBalancedLoanThreshold;
        uint256 buffer;
    }
    ...
    function leverage(address token_, uint256 amount_) external payable {
        ...
        uint256 fee0_;
        uint256 fee1_;
        (amount0_, fee0_) = LendingPool(lendingPool).sendAsset(
            token_,
            collateralInfo_.tokenInfo.assets[0],
            amount0_,
            address(this)
        );
        (amount1_, fee1_) = LendingPool(lendingPool).sendAsset(
            token_,
            collateralInfo_.tokenInfo.assets[1],
            amount1_,
            address(this)
        ...
        uint256 amountLToken0 = ((amount0_ + fee0_) * DECIMAL_FACTOR) / price0_;
        borrowerBalance_.debt0 += amountLToken0;
        uint256 amountLToken1 = ((amount1_ + fee1_) * DECIMAL_FACTOR) / price1_;
    ...
    function CheckLoanCondition(
        ...
        (uint256 minCollateralFactor_, ) = liquidationThreshold
            .calculateLiquidationThreshold(
                liquidationThresholdInfo_.balancedLoanThreshold,
                liquidationThresholdInfo_.unBalancedLoanThreshold,
                liquidationThresholdInfo_.buffer,
                loan0_,
                loan1_
            );
        require(
    ...
    function getMaxLeverageFactor(
        ...
        if (totalLoan_ == 0) {
            ...
            LiquidationThresholdInfo
                memory liquidationThresholdInfo_ = collateralInfos[token_]
                    .liquidationThresholdInfo;
            (uint256 minColateralactor_, ) = liquidationThreshold
                .calculateLiquidationThreshold(
                    liquidationThresholdInfo_.balancedLoanThreshold,
                    liquidationThresholdInfo_.unBalancedLoanThreshold,
                    liquidationThresholdInfo_.buffer,
            ...
            if (minColateralactor_ == DECIMAL_FACTOR) {
                return 1000 * DECIMAL_FACTOR;
            } else if (minColateralactor_ < DECIMAL_FACTOR) {
                return 0;
            } else {
                return
                    (minColateralactor_ * DECIMAL_FACTOR) /
                    (minColateralactor_ - DECIMAL_FACTOR);
            }
        }
    }
    ...
    function getPositionData(DataIn memory dataIn_) 
        public view returns (DataOut memory data_){

        (, , uint256 tokenPrice_) = collateralPriceOracle.getCollateralPrice(
             dataIn_.token
        );
        BorrowerBalance memory borrowerBalance_ = borrowerBalances[ dataIn_.borrower][ dataIn_.token];
        if( dataIn_.direction){
            data_.collateral = borrowerBalance_.collateral +  dataIn_.amount;
        }
        else {
            data_.collateral = borrowerBalance_.collateral -  dataIn_.amount;
        }
         data_.collateralValue = (tokenPrice_ * data_.collateral) / DECIMAL_FACTOR;

        TokenInfo memory tokenInfo_ = collateralInfos[ dataIn_.token].tokenInfo;
        address asset_;
        ...
        if ( dataIn_.direction0){
           data_.debt0 = (lTokenPrice_ * borrowerBalance_.debt0  / DECIMAL_FACTOR ) +  dataIn_.amount0;
        } else {
             data_.debt0  = (lTokenPrice_ * borrowerBalance_.debt0 / DECIMAL_FACTOR) -  dataIn_.amount0;
        }
         data_.debtValue0 = (assetPriceOracle.getAssetPrice(asset_) * data_.debt0) / DECIMAL_FACTOR;

        asset_ = tokenInfo_.assets[1];
        (lTokenPrice_, ) = LendingPool(lendingPool).getTokenPrice(asset_);
          if (dataIn_.direction1){
            data_.debt1 = (lTokenPrice_ * borrowerBalance_.debt1  / DECIMAL_FACTOR ) +  dataIn_.amount1;
        } else {
            data_.debt1 = (lTokenPrice_ * borrowerBalance_.debt1 / DECIMAL_FACTOR) -  dataIn_.amount1;
        }
        data_.debtValue1 = (assetPriceOracle.getAssetPrice(asset_) * data_.debt1) / DECIMAL_FACTOR;

        if((data_.debtValue0 + data_.debtValue1) != 0){
            data_.collateralFactor = (data_.collateral * DECIMAL_FACTOR) / data_.debtValue0 + data_.debtValue1;
        }
        else {
        ...
        (, data_.liquidationFactor) = liquidationThreshold
            .calculateLiquidationThreshold(
                 collateralInfos[ dataIn_.token].liquidationThresholdInfo.balancedLoanThreshold,
                 collateralInfos[dataIn_.token].liquidationThresholdInfo.unBalancedLoanThreshold,
                 collateralInfos[dataIn_.token].liquidationThresholdInfo.buffer,
                data_.debtValue0,
                data_.debtValue1
            );
        ...
        data_.leverageFactor = (data_.collateralFactor * DECIMAL_FACTOR) /
